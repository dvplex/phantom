@if ($paginator->hasPages())
    <ul class="pagination pagination-no-border" role="navigation">
        {{-- Previous Page Link --}}
...
<div class="pagination-legend" style="margin:15px;font-size:0.6rem">
    ...
    <strong>{{ $paginator->lastItem() }}</strong> {{__('messages.of')}} <b>{{ $paginator->total() }}</b> {{ __('messages.records') }} <br>{{ __('messages.show' ) }}
</div>
