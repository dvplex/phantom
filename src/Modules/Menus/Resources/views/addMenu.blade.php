<form method="post" :action="formPath('/{{ app()->getLocale() }}/admin/menus')" @submit.prevent="onSubmit" @keydown="form.errors.clear($event.target.name)">
    @csrf
    <div class="card">
        <div class="card-body">
            ...
            @if(session('phantom.preferences.modules_sidemenu')=='on')
                <div class="form-group">
                    <label>{{ __('menus::messages.Module') }}</label>
                    <v-select class="form-control" id="modules" v-model="form.formData.module" :options="{{ $modules }}"></v-select>
                    <span class="text-danger" v-if="form.errors.has('module')" v-text="form.errors.get('module')"></span>
                </div>
            @endif
            <div class="form-group">
